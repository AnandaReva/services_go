...
type requestBody1 struct {
	Data         Data1  `json:"data" binding:"required" extensions:"x-order=1"`        // x-order=1 untuk Data
...
// @Model Data
type Data1 struct {
	RowLength  int    `json:"row_length" binding:"required" extensions:"x-order=1"`  // x-order=1 untuk RowLength
	Page       int    `json:"page" binding:"required" extensions:"x-order=2"`        // x-order=2 untuk Page
	SortColumn int    `json:"sort_column" binding:"required" extensions:"x-order=3"` // x-order=3 untuk SortColumn
	Direction  string `json:"direction" binding:"required" extensions:"x-order=4"`   // x-order=4 untuk Direction
	BotID      string `json:"bot_id" binding:"required" extensions:"x-order=5"`      // x-order=5 untuk BotID
}

// GetBotConversationHistoryTable retrieves bot conversation history based on bot ID.
//
...
// @Accept       json
// @Produce      json
// @Param        requestBody1   body  requestBody1 true  "Request Body"
...
// @Router       /get_bot_conversation_history_table [post]
// @Extensions x-order=1
func GetBotConversationHistoryTable(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	referenceId := utils.GlobalVarInstance.GetReferenceId()
	if referenceId == "" {
		referenceId = "undefined"
	}

	utils.Log(referenceId, "\nExecuting method: GetBotConversationHistoryTable")

	// Ambil endpoint backend dari environment variables
	realBackendURL := os.Getenv("endpoint1")
	if realBackendURL == "" {
	...
	client := &http.Client{}
	requestBody1, err := json.Marshal(reqBody) // Use reqBody for backend
	if err != nil {
		errorResponse := map[string]interface{}{
			"error_code":    5000002,
			"error_message": "internal server error",
	...
	req, err := http.NewRequest("POST", realBackendURL, bytes.NewBuffer(requestBody1))
	if err != nil {
		errorResponse := map[string]interface{}{
			"error_code":    5000003,
			"error_message": "internal server error",
