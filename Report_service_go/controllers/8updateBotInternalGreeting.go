...
type requestBody8 struct {
	...
	Greeting string `json:"greeting" binding:"required" extensions:"x-order=2"`
	Topics   string `json:"topics" binding:"required" extensions:"x-order=3"`
}

// UpdateBotInternalKnowledge update bot internal greeting based on bot ID.
//
...
// @Accept       json
// @Produce      json
// @Param        requestBody8   body  requestBody8 true  "Request Body"
...
// @Router       /update_bot_internal_greeting [post]
// @Extensions x-order=8
func UpdateBotInternalGreeting(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	referenceId := utils.GlobalVarInstance.GetReferenceId()
	if referenceId == "" {
		referenceId = "undefined"
	}

	utils.Log(referenceId, "\nExecuting method: UpdateBotInternalGreeting")

	// Ambil endpoint backend dari environment variables
	realBackendURL := os.Getenv("endpoint8")
	if realBackendURL == "" {
