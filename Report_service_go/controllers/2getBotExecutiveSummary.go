...
type requestBody2 struct {
	Data     Data2 `json:"data" binding:"required" extensions:"x-order=1"`      // x-order=1 untuk Data
...
// GetBotExecutiveSummary retrieves bot executive summary based on bot ID.
//
...
// @Accept       json
// @Produce      json
// @Param        requestBody2   body  requestBody2 true  "Request Body"
...
// @Router       /get_bot_executive_summary [post]
// @Extensions x-order=2
func GetBotExecutiveSummary(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	referenceId := utils.GlobalVarInstance.GetReferenceId()
	if referenceId == "" {
		referenceId = "undefined"
	}

	utils.Log(referenceId, "\nExecuting method: GetBotExecutiveSummary")

	// Ambil endpoint backend dari environment variables
	realBackendURL := os.Getenv("endpoint2")
	if realBackendURL == "" {
