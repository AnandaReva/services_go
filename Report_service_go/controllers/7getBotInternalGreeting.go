...
// GetBotInternalGreeting update bot internal Greeting based on bot ID.
//
...
// @Accept       json
// @Produce      json
// @Param        requestBody7   body  requestBody7 true  "Request Body"
...
// @Router       /get_bot_internal_greeting [post]
// @Extensions x-order=7
func GetBotInternalGreeting(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	referenceId := utils.GlobalVarInstance.GetReferenceId()
	if referenceId == "" {
		referenceId = "undefined"
	}

	utils.Log(referenceId, "/nExecuting method: GetBotInternalGreeting")

	// Ambil endpoint backend dari environment variables
	realBackendURL := os.Getenv("endpoint7")
	if realBackendURL == "" {
