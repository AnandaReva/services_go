...
// @Tags         Bot Inital Data
// @Accept       json
// @Produce      json
// @Param        requestBody9   body  requestBody9 true  "Request Body"
...
// @Router       /get_initial_data [post]
// @Extensions x-order=9
func GetInitialData(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	referenceId := utils.GlobalVarInstance.GetReferenceId()
	if referenceId == "" {
		referenceId = "undefined"
	}

	utils.Log(referenceId, "/nExecuting method: GetInitialData")

	// Ambil endpoint backend dari environment variables
	realBackendURL := os.Getenv("endpoint9")
	if realBackendURL == "" {
