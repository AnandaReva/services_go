...
type requestBody6 struct {
	BotID              string `json:"bot_id" binding:"required" extensions:"x-order=1"`
	ChildPromptID      string `json:"child_prompt_id" binding:"required" extensions:"x-order=2"`
	KnowledgeText      string `json:"knowledge_text" binding:"required" extensions:"x-order=3"`
	ClassificationName string `json:"classification_name" binding:"required" extensions:"x-order=4"`
}

// UpdateBotInternalKnowledge update bot internal konwledge based on bot ID.
//
...
// @Accept       json
// @Produce      json
// @Param        requestBody6   body  requestBody6 true  "Request Body"
...
// @Router       /update_bot_internal_knowledge [post]
// @Extensions x-order=7
func UpdateBotInternalKnowledge(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	referenceId := utils.GlobalVarInstance.GetReferenceId()
	if referenceId == "" {
		referenceId = "undefined"
	}

	utils.Log(referenceId, "\nExecuting method: UpdateBotInternalKnowledge")

	// Ambil endpoint backend dari environment variables
	realBackendURL := os.Getenv("endpoint6")
	if realBackendURL == "" {
